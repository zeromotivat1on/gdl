...
{
    table_init(&g_sid_table, arena, max_sid_count, sizeof(u64), max_sid_size, (table_hash_func)sid_table_hash_func);
}
...
{
    return (const char*)table_find(&g_sid_table, &n);
}

inline bool is_unique_sid(sid n)
{
    return table_find(&g_sid_table, &n) == nullptr;
}
